...
export class ProcessComponent implements OnInit {
    ...
    public getIndex(search){
        ...
        merge(this.paginator.page)
            .pipe(
                ...
                switchMap(() => {
                    this.loading = true;
                    return this.authenticationService.get('process',
    ...
    public add(){
        this.router.navigate(['administrator/process/add']);
    }

    public edit(id){
        this.router.navigate(['administrator/process/edit/' + id]);
    }

    public delete(id){
        console.log("Delete Process");
        this.loading = true;
        this.authenticationService.delete('process/' + id).subscribe(
            payload => {
                this.loading = false;
                this.dialogInfo("Información", "El Proceso se eliminó correctamente");
                this.getIndex('');
            },
            (error) => {
                if(error.status == 422){
    ...
    public dialogConfirm(id) {
        let dialogRef = this.dialog.open(DialogConfirmComponent, {
            maxWidth: '800px',
            height: 'auto',
            data: {title: 'Información', description: 'Estas seguro de eliminar este Proceso'}
        });
