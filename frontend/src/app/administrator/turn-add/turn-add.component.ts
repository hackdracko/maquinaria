...
@Component({
  selector: 'app-turn-add',
  templateUrl: './turn-add.component.html',
  styleUrls: ['./turn-add.component.css']
})
export class TurnAddComponent implements OnInit {
    /**
     * Indicates FormGroup
     * @type {FormGroup}
     */
    ...
    public back() {
        this.router.navigate(['administrator/turn']);
    }
    ...
    public dialogConfirm() {
        let dialogRef = this.dialog.open(DialogConfirmComponent, {
            maxWidth: '800px',
            height: 'auto',
            data: {title: 'Información', description: 'Estas seguro de crear un nuevo Turno'}
        });
    ...
    public save() {
        ...
        this.authenticationService.post('turn', data).subscribe(
            payload => {
                this.loading = false;
                this.formUnlock();
                this.dialogInfo("Información", "El Turno se creo correctamente");
                this.router.navigate(['administrator/turn']);
            },
            (error) => {
                if (error.status == 422) {
