...
export class TurnEditComponent implements OnInit {
    ...
    public dialogConfirm() {
        let dialogRef = this.dialog.open(DialogConfirmComponent, {
            maxWidth: '800px',
            height: 'auto',
            data: {title: 'Información', description: 'Estas seguro de editar el Turno'}
        });
    ...
    public getInfo() {
        console.log("Info");
        this.loading = true;
        this.authenticationService.get('turn/' + this.id, '').subscribe(
            payload => {
                this.loading = false;
                this.result = payload;
                this.form();
    ...
    public save() {
        ...
        this.authenticationService.put('turn/' + this.id, data).subscribe(
            payload => {
                this.loading = false;
                this.formUnlock();
                this.dialogInfo("Información", "El Turno se edito correctamente");
                this.router.navigate(['administrator/turn']);
            },
            (error) => {
                if (error.status == 422) {
