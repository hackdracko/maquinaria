...
export class TurnComponent implements OnInit {
    ...
    public getIndex(search){
        ...
        merge(this.paginator.page)
            .pipe(
                ...
                switchMap(() => {
                    this.loading = true;
                    return this.authenticationService.get('turn',
    ...
    public add(){
        this.router.navigate(['administrator/turn/add']);
    }

    public edit(id){
        this.router.navigate(['administrator/turn/edit/' + id]);
    }

    public delete(id){
        console.log("Delete Turn");
        this.loading = true;
        this.authenticationService.delete('turn/' + id).subscribe(
            payload => {
                this.loading = false;
                this.dialogInfo("Información", "El Turno se eliminó correctamente");
                this.getIndex('');
            },
            (error) => {
                if(error.status == 422){
    ...
    public dialogConfirm(id) {
        let dialogRef = this.dialog.open(DialogConfirmComponent, {
            maxWidth: '800px',
            height: 'auto',
            data: {title: 'Información', description: 'Estas seguro de eliminar el Turno'}
        });
