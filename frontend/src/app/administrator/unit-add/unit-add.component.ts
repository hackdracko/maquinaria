...
export class UnitAddComponent implements OnInit {
    ...
    public dialogConfirm() {
        let dialogRef = this.dialog.open(DialogConfirmComponent, {
            maxWidth: '800px',
            height: 'auto',
            data: {title: 'Información', description: 'Estas seguro de crear una nueva Unidad'}
        });
    ...
    public save() {
        ...
        this.authenticationService.post('unit', data).subscribe(
            payload => {
                this.loading = false;
                this.formUnlock();
                this.dialogInfo("Información", "La unidad se creo correctamente");
                this.router.navigate(['administrator/unit']);
            },
            (error) => {
                if (error.status == 422) {
